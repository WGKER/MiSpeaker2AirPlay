...
def create_web_app(config: Config, app_instance) -> web.Application:
    ...
    async def handle_qr_start(request):
        """开始二维码登录，生成qr链接"""
        auth = app_instance.auth
        try:
            await auth._gen_qr_login()
            return web.json_response({
                "qr_url": auth.qr_data["qr"],
                "lp_url": auth.qr_data["lp"]
            })
        except Exception as e:
            log.exception("生成二维码失败")
            return web.json_response({"error": str(e)}, status=500)

    async def handle_qr_poll(request):
        """轮询扫码结果"""
        auth = app_instance.auth
        try:
            res = await auth._poll_qr_result(auth.qr_data["lp"], timeout=10)
            return web.json_response({"code":0, "userId": res["userId"], "passToken": res["passToken"]})
        except Exception as e:
            msg = str(e)
            if "二维码已过期" in msg:
                return web.json_response({"code":86002, "msg":msg})
            elif "等待扫码" in msg:
                return web.json_response({"code":86001, "msg":"等待扫码"})
            return web.json_response({"code":-1, "msg":msg})
